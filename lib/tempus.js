    ...
    this.patterns = [
        {
            pattern: /d{2}-m{2}-y{4}/,
            formatter: date => {
                let extractedTime = this.extractDate(date);
                let d = extractedTime.day.str;
                let m = extractedTime.month.str.long;
                let y = extractedTime.year.str;
                return `${d}-${m}-${y}`;
            }
        },
        {
...
Tempus.prototype.isLeapYear = function () {
...
Tempus.prototype.add = function (year = 0, month = 0, week = 0, day = 0, hour = 0, min = 0, sec = 0) {
    /**
     * TODO
     * * test this out
     * * recursion
     * * check year for every yearly changes then re-calculate secs|hours|days|months etc...
     */
    let ms = this.times.ms;
    let s = sec * this.times.sec * ms;
    let m = min * this.times.min * ms;
    let h = hour * this.times.hour * ms;
    let d = day * this.times.day * ms;
    let w = week * this.times.week * ms;
    let _m = month * this.times.month * ms;
    let y = year * this.times.year * ms;
    return new Tempus(new Date(this.unix + s + m + h + d + w + _m + y), this.language);
}
...
Tempus.prototype.diffrence = function (date) {
    ...
    const ms = this.times.ms;
    const diff = (t1 - t2) / ms;
    return {
        sec: Math.round(diff / this.times.sec),
        min: Math.round(diff / this.times.min),
        hour: Math.round(diff / this.times.hour),
        day: Math.round(diff / this.times.day),
        week: Math.round(diff / this.times.week),
        month: Math.round(diff / this.times.month),
        year: Math.round(diff / this.times.year)
    }
}
